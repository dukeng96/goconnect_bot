...
from message_config import START, HELP, SOOTHING_ASK_ENV, NEED_INFO_USER, FIXED_USER, GROUP_BUG_TEMPLATE, GROUP_FIX_SUFFIX
from db import (
    now_iso, create_ticket, add_event, set_status, update_meta, add_attachment,
    upsert_user, latest_open_draft, reset_user_flow
)
from llm import classify, extract_entities, rag_answer
from storage import upload_bytes, guess_content_type
from formatting import ticket_preview_text, fmt_attachments
...
async def _forward_to_group(context: ContextTypes.DEFAULT_TYPE, tid: str, src_msg: str, user, ticket_doc):
    ...
    text = GROUP_BUG_TEMPLATE.format(
        ticket_id=tid, username=(user.username or 'user'), user_id=user.id, text=src_msg,
        platform=meta.get('platform','unknown'), os=meta.get('os','unknown'), app_version=meta.get('app_version','unknown'),
        attachments=fmt_attachments(ticket_doc.get('attachments', [])), time_utc=datetime.utcnow().strftime('%H:%M:%S UTC')
    )
    kb = InlineKeyboardMarkup([[ InlineKeyboardButton("Đã khắc phục", callback_data=f"fix:{tid}") ]])
    await context.bot.send_message(chat_id=INCIDENT_GROUP_ID, text=text, reply_markup=kb, disable_web_page_preview=True, parse_mode='HTML')
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'fix':
        ...
        row = context.application.bot_data.get(tid)
        # DM user
        from db import col_tickets
        data = col_tickets.find_one({'id': tid}, {'chat_id':1})
        if data:
            await context.bot.send_message(chat_id=data['chat_id'], text=FIXED_USER, parse_mode='HTML')
        ...
        await q.message.reply_text("Dạ, em đã chuyển thông tin cho team kỹ thuật, team sẽ kiểm tra ngay và phản hồi sớm ạ.")
        ...
        await q.message.reply_text("Dạ, mời anh/chị gửi thêm ảnh/video/miêu tả chi tiết để em cập nhật vào phiếu ạ.", parse_mode='HTML')

# --- Message pipeline ---
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.effective_message; user = msg.from_user
    text = msg.text or msg.caption or ''
    upsert_user(user.id, user.username, user.full_name)

    draft = latest_open_draft(user.id)

    # 1) Nếu đang DRAFT → classifier trước
    if draft and draft.get('status') == 'DRAFT' and text:
        cls = classify(text)
        if cls.get('class') == 'QNA' and cls.get('confidence', 0) >= 0.6:
            set_status(draft['id'], 'CANCELLED'); add_event(draft['id'], str(user.id), 'cancel_by_qna')
            answer = rag_answer(user.id, text)
            await msg.reply_text(answer, parse_mode='Markdown', disable_web_page_preview=True)
            return
        # không phải QNA → cập nhật meta và show preview
        meta = extract_entities(text)
        update_meta(draft['id'], meta)
        from db import col_tickets
        t = col_tickets.find_one({'id': draft['id']})
        kb = InlineKeyboardMarkup([[ InlineKeyboardButton("Xác nhận phiếu", callback_data=f"user_confirm:{t['id']}"),
                                     InlineKeyboardButton("Cập nhật phiếu", callback_data=f"user_update:{t['id']}") ]])
        await msg.reply_text(ticket_preview_text(t), reply_markup=kb, disable_web_page_preview=True, parse_mode='HTML')
        return

    # 2) Nếu đang UPDATING và người dùng gửi file → upload MinIO
    if draft and draft.get('status') == 'UPDATING' and (msg.photo or msg.document or msg.video):
        if msg.photo:
            file_id = msg.photo[-1].file_id; filename = f"photo_{msg.photo[-1].file_unique_id}.jpg"
        elif msg.document:
            file_id = msg.document.file_id; filename = msg.document.file_name or f"doc_{msg.document.file_unique_id}"
        else:
            file_id = msg.video.file_id; filename = msg.video.file_name or f"video_{msg.video.file_unique_id}.mp4"
        tgfile = await context.bot.get_file(file_id)
        bio = io.BytesIO(); await tgfile.download_to_memory(out=bio); bio.seek(0)
        from storage import upload_bytes, guess_content_type
